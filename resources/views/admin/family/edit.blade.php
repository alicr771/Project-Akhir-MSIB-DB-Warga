    ...
    <div class="container">
        <div class="card rounded shadow">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="h2 mb-0">Edit Kartu Keluarga</h4>
            </div>
            <form action="{{ route('family.update', $data->id) }}" method="POST" class="card-body">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="number" class="form-label">NIK</label>
                    <input type="text" name="number" class="form-control" id="number" placeholder="NIK"
                        value="{{ old('number', $data->number) }}">
                </div>
                <div class="mb-3">
                    <select name="resident_id" id="resident_id"
                        class="form-control @error('resident_id') is-invalid @enderror" required>
                        <option value="">{{ __('Pilih Kepala Keluarga') }}</option>
                        @foreach ($residents as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $data->resident->id ? 'selected' : '' }}>{{ $item->nik . ' - ' . $item->name }}</option>
                        @endforeach
                    </select>
